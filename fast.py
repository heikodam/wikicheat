...
def get_all_reverse_degree_links(degree, start_degree_pages, start_link, path):
    current_degree_pages = set()
    #if no more degrees to go return all links of current degree
    if degree == 0:
        return start_degree_pages
    ...
    return get_all_reverse_degree_links(degree - 1, current_degree_pages, start_link, path)


#set the start and end wiki page title
start_link = "Germany".lower()
end_link = "Trypsin".lower()
...
while degree_count < 5:
    ...
    reverse_path.append(get_all_reverse_degree_links(1, reverse_path[degree_count - 1], start_link, reverse_path))
    # print(reverse_path)
    degree_count += 1
